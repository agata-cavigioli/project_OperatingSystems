...
#define ENTRYHI_GET_VPN(x) (((x) & ENTRYHI_VPN_MASK2) == VPNSTACK ? USERPGTBLSIZE-1 : ((x) - VPNSTARTADDR) >> ENTRYHI_VPN_BIT)
#define ENTRYLO_SET_PFN(reg, x) reg = (reg & ~ENTRYLO_PFN_MASK) | (FLASHPOOLSTART + ((x) << ENTRYLO_PFN_BIT))

/* End of our utility constants */
